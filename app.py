...
def mark_ai_request_failed(
    ...
    """
    AI或诊断流程失败时，将pending请求结算为failed。

    failed请求不会增加daily_usage成功调用次数。
    """
...
with st.sidebar:
    ...
    st.write(
        f"今日成功调用："
        f"`{usage_status['used']}` 次"
    )

    if usage_status["pending"] > 0:
        st.write(
            f"正在处理："
            f"`{usage_status['pending']}` 次"
        )
...
def format_message_meta(
    ...
    if meta.get("usage_limit_reached"):
        parts.append(
            "未调用AI：已达到每日上限"
        )
...
if user_question:
    ...
    try:
        ...
        request_id = (
            f"message-{saved_user_message['id']}"
        )
    ...
    with st.chat_message("assistant"):
        with st.spinner(
            ...
            try:
                ...
                logger.info(
                    "聊天处理完成，用户ID=%s，会话ID=%s，模式=%s，request_id=%s",
                    current_user_id,
                    current_conversation_id,
                    current_work_mode,
                    request_id,
                ...
                logger.error(
                    "AI服务错误，用户ID=%s，request_id=%s，错误=%s",
                    current_user_id,
                    request_id,
                    error,
                )
                ...
                logger.error(
                    "PLC连接失败，用户ID=%s，request_id=%s，错误=%s",
                    current_user_id,
                    request_id,
                    error,
                )
                ...
                logger.warning(
                    "用户访问无权限会话，用户ID=%s，request_id=%s，错误=%s",
                    current_user_id,
                    request_id,
                    error,
                )
                ...
                logger.exception(
                    "请求处理异常，用户ID=%s，request_id=%s，错误=%s",
                    current_user_id,
                    request_id,
                    error,
                )
